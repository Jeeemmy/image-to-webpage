...
Infer the adaptation width before the first reply:

- Read the screenshot's source pixel dimensions when available. If dimensions cannot be read directly, estimate from the image and state the uncertainty.
- Identify presentation wrappers, browser/device mock frames, and OS/system chrome. If any wrapper is ignored, infer from the effective real product UI width rather than the full screenshot width.
- Decide whether the screenshot appears to be 1:1 CSS pixels or a scaled/high-DPI export. Use visible control sizes, typography, common device/browser chrome proportions, and app density as evidence.
- For PC/desktop and mobile alike, choose the most plausible calibration width from the screenshot evidence; do not force a device-class default. When multiple widths are plausible, include the strongest alternate candidate and why.
- Once the user confirms or corrects the width, treat it as `confirmed_adaptation_width` for scale calculation and artifact metadata.

The first reply must include only these confirmation items:

- The recognized target device type: PC/desktop or mobile.
- The inferred adaptation width for the recognized device, derived from the screenshot rather than a fixed default. Use visual evidence such as source pixel dimensions, ignored wrapper/effective product UI bounds, common desktop or mobile viewport proportions, visible device/browser chrome, OS status/navigation bars, and whether the screenshot appears to be a scaled export. If the user already specified a width, show the user's width instead and mark it as user-provided.
- The concise basis for the inferred adaptation width, including the source screenshot size or effective product UI width when it is known from the attached image metadata/inspection, and any uncertainty or alternate plausible width. State that this width is the fidelity calibration point, not a fixed page/root, phone, or artboard width; the rendered page must still adapt to the user's viewport.
...
End the first reply by asking the user to confirm or correct the inferred width before reconstruction starts. After sending the first reply, STOP and wait for an explicit user confirmation, such as "确认", "继续", "OK", "按这个做", or a corrected width/target followed by approval.
...
Record viewport adaptation intent in the DSL request metadata. The adaptation width is the user-confirmed measurement baseline inferred from the screenshot for scale and fidelity at the confirmed width; it must not become a fixed root/artboard width, fixed screenshot-height page, or non-responsive viewport lock.
...
For every screenshot crop or crop reference, record crop bounds as absolute source screenshot pixel coordinates. The confirmed adaptation width and normalized render scale are only for CSS/layout measurements; they must not be applied to crop x/y/width/height. Do not infer crop size from the eventual CSS display size. Record the asset's source crop size and the intended CSS display size as separate values when they differ, such as a `69px x 69px` source avatar displayed at `36px x 36px`.
...
Before rendering, verify that the First Response Confirmation Gate has completed and the user has explicitly confirmed or corrected the inferred adaptation width after seeing the first reply:

- Tell the user whether the screenshot was recognized as PC/desktop or mobile.
- Infer the adaptation width from the screenshot for both PC/desktop and mobile sources; do not use a fixed default width for either device class.
- Tell the user which inferred width will be used in the first reply and why.
...
- Do not hard-code a root/page/artboard to the confirmed width or screenshot dimensions, for example `width: <confirmed_width>px`, a matching fixed screenshot height, or a centered fixed-size screenshot stage, unless the user explicitly requested a static artboard export. Use fixed pixel values only for measured component details, true product-owned fixed panels, or max/min constraints that sit inside a responsive shell.
- For desktop document/marketing pages, prefer `width: 100%`/`100vw`, `min-height: 100vh` or dynamic viewport units, flex/grid tracks, `minmax()`, `min()`, `max()`, `clamp()`, and content-level `max-width` constraints as needed.
- For desktop dashboard/app-shell pages with persistent sidebar/topbar chrome, `min-height: 100vh`/`100dvh` alone is not sufficient. Use a viewport-bounded shell (`height: 100vh`/`100dvh` or equivalent), prevent document/body scrolling for that shell with `overflow: hidden`, keep every flex/grid ancestor in the scroll chain `min-height: 0`, and make exactly the intended content pane the vertical scroll owner (`overflow-y: auto`). Sidebar/topbar persistence is not proven by `position: sticky` unless the browser document remains at `scrollY = 0` while the content pane scrolls.
- For mobile, prefer dynamic viewport sizing, safe-area-aware persistent chrome, and scrollable content panes instead of fixed phone-height pages.
...
Include source artifacts, confirmed adaptation width, width inference evidence, scale, files changed, viewport adaptation strategy, scroll architecture, verification commands, style/asset integrity checks, and browser checks. For dashboard/app-shell layouts, the render artifact must record the primary scroll owner selector, whether document/body scrolling is disabled for the shell, root viewport-height and overflow rules, min-height-zero chain coverage, persistent chrome selectors, and the result of a browser scroll test or a non-browser CSS scroll-invariant gate.

For DSL image nodes with `asset_strategy = "generate_transparent_subject"`, use the user's available image generation skill with the screenshot crop/reference to preserve the subject while removing background and interactive elements, then render any background gradients in code and overlay controls separately. If image generation is available and not explicitly declined, do not downgrade a separable important image to `source_crop`; correct the DSL/render plan instead. For `asset_strategy = "source_crop"`, create/use the recorded whole-element crop only as a fallback final asset when image generation is unavailable, the user declined image generation, or the DSL documents why transparent separation is unsuitable. Before saving a `source_crop`, inspect the crop bounds against the screenshot and artifact: if the crop includes device/browser chrome, status bars, iOS/Android system navigation indicators, notches, navigation buttons, bottom sheets, text/cards, badges, or any other overlay that should not be part of the underlying image, reject that source crop and switch to `generate_clean_asset` or a clean crop that excludes contaminants. Crop final source assets from the original screenshot pixel grid with a pixel-copy/bitmap-clone API, never with drawing or resampling APIs such as `Graphics.DrawImage`, canvas `drawImage`, CSS screenshots, or display-size recrops. After saving the crop, compare the output pixels against the original source rectangle and require width/height to match the source rectangle and `pixel_mismatches = 0`; discard and recrop if the check fails. For `asset_strategy = "generate_clean_asset"`, use the DSL prompt features or reference crop to generate a clean non-layered asset only when transparent subject extraction is not supported or not appropriate; prompts must explicitly ask to remove all detected contaminating UI and presentation chrome.
...
- For desktop dashboard/app-shell pages when browser/page opening is unavailable, inspect the built CSS/source and confirm the scroll-invariant rules exist: the shell is bounded to the viewport with `height: 100vh`/`height: 100dvh` or equivalent, the shell clips document overflow with `overflow: hidden`, intermediate grid/flex panes use `min-height: 0`, the intended main pane uses `overflow-y: auto`, and persistent chrome has an opaque background plus sticky/fixed positioning. Do not accept a result whose only root height rule is `min-height: 100vh`/`min-height: 100dvh`.
...
- Infer the adaptation width from the screenshot and ask the user to confirm or correct it before generating artifacts. Do not use fixed PC/mobile defaults.
- Do use raw source screenshot pixels for asset crop coordinates. Do not multiply crop coordinates or dimensions by `adaptation_width / screenshot_pixel_width`, and do not reverse-engineer a crop from CSS display dimensions.
- Compute normalized scale as `confirmed_adaptation_width / effective_source_width`. `effective_source_width` is the inner real UI width when a presentation wrapper is ignored, otherwise it is `screenshot_pixel_width`.
- The confirmed adaptation width is a fidelity calibration baseline, not a fixed output resolution. Do not make the page root, app shell, body, or primary scroll container a fixed confirmed-width or screenshot-height artboard unless the user explicitly asked for a static artboard export.
...
- For side-anchored text/link stacks such as hero support copy, contact links, helper panels, account prompts, and right/left rail text blocks, capture both the container's edge anchor and the internal alignment. A stack whose right edge aligns to a safe-area boundary and whose text is right-aligned must record end alignment and right text alignment; do not render it as a right-positioned but left-aligned column by default.
...
- Treat exact source cropping as a pixel-copy operation. Use APIs such as bitmap clone/crop or Pillow `Image.crop` that copy the decoded source rectangle without interpolation. Avoid drawing APIs even when source and destination rectangles have the same dimensions, because interpolation and pixel-center offsets can pull pixels from outside the intended rectangle. Verify saved crop pixels against the original screenshot rectangle and record `pixel_mismatches = 0` in the render artifact for final `source_crop` assets.
...
- For repeated carousel/list image items that are partially clipped by the screenshot edge, do not keep the clipped raster as the final reusable image asset when image generation is available and not declined. Use visible siblings in the same template plus the clipped fragment as references, infer/reconstruct a complete subject, and output a full subject asset (prefer transparent background) that matches sibling style/perspective/lighting. Keep the card-level clipping as a layout effect, not as irreversible asset damage.
- For generated transparent subjects used in cards/lists, enforce edge cleanliness in generation and post-processing: remove text/UI overlays, remove matte/halo/fringe, avoid white/black edge residues, and keep a complete silhouette with transparent padding. If edge contamination remains, regenerate or run cleanup (`despill`, edge-contract/feather) before accepting the asset.
...
- For horizontal product/card carousels, measure each repeated item's outer card bounds independently from its image/media crop. Record the item width, height, and aspect ratio in tokens and DSL, then render cards from those outer dimensions. Do not infer card height from the visible media crop, from the viewport slice, or from generic card defaults; if sibling carousel cards share the same template, propagate the measured item dimensions across the row.
...
- For repeated cards/list items with a separated bottom action strip, capture and render the strip as its own non-shrinking footer region. This includes settings/action icon buttons, "Details"/row-action buttons, toggles/switches, bottom dividers, footer padding, and left/right action distribution. Do not flatten these controls into the card body, infer them only from generic button tokens, or let fixed card heights/`overflow-hidden` crop the footer controls.
